...
static uint32_t keymap[256] = {
  // [SDL_SCANCODE_ESCAPE] = _KEY_ESCAPE, [SDL_SCANCODE_F1] = _KEY_F1, ...
...
printf("keybord.c: %s\tkey_queue[key_r(%d)] = am_scancode(%u)\n", am_scancode & KEYDOWN_MASK ? "down" : "up", key_r, am_scancode & ~KEYDOWN_MASK);
...
static void i8042_data_io_handler(uint32_t offset, int len, bool is_write) {
  ...
  assert(offset == 0);
  if (key_f != key_r) {
printf("keybord.c: call keyboard handler\n");
