  ...
  while (e[position] != '\0')
  {
    ...
    for (i = 0; i < NR_REGEX; i++)
    {
      ...
      if (ret == 0 && pmatch.rm_so == 0)
      {
        ...
        default:
          Log("no match rules like \"%d\" type\n at position %d with len %d: %.*s", rules[i].token_type, position, substr_len, substr_len, substr_start);
        }
...
int find_main_op(int p, int q, int* state)
{
  ...
  while (p <= q)
  {
    ...
    case TK_RP:
      ...
      if(level < 0)
      {
        printf("right parenthesis cant be less than left\n");
        *state = FIND_OP_FAIL;
        return -1;
      }
      break;
...
word_t eval(int p, int q, int *state)
{
  ...
  else
  {
    ...
    case TK_DEREF:
      if(p != op_pos)
      {
        printf("dereference* and negative- should be on the left\n");
        *state = BAD_EXPR;
        return -1;
      }
    ...
    default:
      printf("impossible main op type %d", tokens[op_pos].type);
      *state = BAD_EXPR;
      return -1;
    }
  }
